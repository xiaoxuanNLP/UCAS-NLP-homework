...
class DM(nn.Module):
    def __init__(self, vec_dim, num_docs, num_words):
        ...
        self._Output = nn.Parameter(
            torch.FloatTensor(vec_dim,num_docs).zero_(),requires_grad=True # 这个地方注意要把输出初始化为0
        )
    ...
    def forward(self,context_ids,doc_ids,target_noise_ids):
        ...
        print("self._Doc = ",self._Doc.shape)
        print("self._Word = ",self._Word.shape)
        print("self._Doc[doc_ids:] = ",self._Doc[doc_ids:].shape)
        print("self._Word[context_ids,:] = ",self._Word[context_ids,:].shape)
        print("x = ",x.shape)
